...
public class HashDictionaryLinear<K, V> implements DictionaryInterface<K, V> 
{
	...
	public V put(K key, V value) 
	{
		...
		if(contains(key))
		{
			...
			{
				hashValue = (hashValue + 1) % currentCapacity;
			}
		...
		{
			result = linearProbing(key, value);
		}
		return result;
	}
	...
	private void rehash()
	{
		...
		dictionary = (HashDictionaryLinear<K, V>.TableElement<K, V>[]) new TableElement[currentCapacity];
		numberOfElements = 0;
