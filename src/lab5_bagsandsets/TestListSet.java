...
public class TestListSet
{
	...
	public void testIterator()
	{
		...
		java.util.Iterator<Integer> itr = s1.iterator();

		// there should be no iteration of an empty set
		assertFalse(itr.hasNext());

		// if we call next() it should throw an exception
		try
		{
			itr.next();
		...
		// get an iterator of the populated set
		itr = s1.iterator();

		// place each element from the set into an ArrayList
		while (itr.hasNext())
			list.add(itr.next());

		// the list should have 10 elements
		assertEquals(10, list.size());

		// make sure the list contains 1 ... 10
		for (int i = 0; i < nums1.length; i++)
			assertTrue(list.contains(nums1[i]));
